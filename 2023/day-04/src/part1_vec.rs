...
This version uses bytes from the string and parses much faster.
Interestingly, giving the capacity of the vecs also increases speed by 30%.
...
impl Card {
    ...
    pub fn winning_numbers_count(&self) -> u32 {
        let count = self
            .drawn_numbers
            .iter()
            .filter(|x| self.winning_numbers.contains(x))
...
pub fn parse_cards(input: &str) -> Vec<Card> {
    let cards: Vec<Card> = input
        ...
        .map(|line| {
            ...
            let mut winning_numbers = Vec::with_capacity(N_WINNING_CARDS);
            for number in b_line[pos_win..pos_draw].split(|&it| it == b' ') {
                match number.len() {
                    0 => continue,
                    1 => winning_numbers.push(number[0] - b'0'),
                    2 => winning_numbers.push((number[0] - b'0') * 10 + number[1] - b'0'),
                    _ => panic!("read error"),
                }
            }
            let mut drawn_numbers = Vec::with_capacity(N_DRAWN_CARDS);
            for number in b_line[pos_draw + 1..].split(|&it| it == b' ') {
                match number.len() {
                    0 => continue,
                    1 => drawn_numbers.push(number[0] - b'0'),
                    2 => drawn_numbers.push((number[0] - b'0') * 10 + number[1] - b'0'),
                    _ => panic!("read error"),
                }
            }
