...
4. Store numbers for a * in a (large) Array to identify cases where two numbers link to a *.
...
pub fn solve_puzzle(input: &str) -> String {
    ...
    // first number for a symbol in an array reflecting the line and row
    let mut symbol_map: [[Value; 142]; 150] = [[0; 142]; 150];
...
/// Returns a vec of found numbers (Symbol work pos, number)
// #[inline]
fn find_numbers_adjacent_asterix(
    ...
    for &pos in asterix_positions[1].iter() {
        if work[1][pos - 1] < 10 {
            ...
            let mut p = pos - 1;
            let mut n = work[1][p] as Value;
            p -= 1;
            let mut factor = 10;
            while work[1][p] < 10 {
                n += work[1][p] as Value * factor;
                factor *= 10;
                p -= 1;
            }
        ...
        for p in pos - 1..pos + 2 {
            if work[0][p] < 10 {
                ...
                q += 1;
                let mut n = work[0][q] as Value;
                q += 1;
                while work[0][q] < 10 {
                    n = n * 10 + work[0][q] as Value;
                    q += 1;
                }
                result_numbers.push((pos, n));
                if q == pos {
        ...
        for p in pos - 1..pos + 2 {
            if work[2][p] < 10 {
                ...
                q += 1;
                let mut n = work[2][q] as Value;
                q += 1;
                while work[2][q] < 10 {
                    n = n * 10 + work[2][q] as Value;
                    q += 1;
                }
                result_numbers.push((pos, n));
                if q == pos {
