...
fn criterion_benchmark_part1(c: &mut Criterion) {
    ...
    group.bench_with_input("part1_regex", &input, |b, input| {
        b.iter(|| part1::solve_puzzle_regex(input))
    });
...
fn criterion_benchmark_part2(c: &mut Criterion) {
    ...
    group.bench_with_input("part2_regex", &input, |b, input| {
        b.iter(|| part2::solve_puzzle_regex(input))
    });
    group.finish();
}
