...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_with_input("parse1 only", &input, |b, input| {
        b.iter(|| parse_data(input, true))
    });
    group.bench_with_input("part1 simple", &input, |b, input| {
        b.iter(|| part1::solve_puzzle_simple(input))
    });
    ...
    group.bench_with_input("parse2 only", &input, |b, input| {
        b.iter(|| parse_data(input, false))
    });
    group.bench_with_input("part2 create vec", &input, |b, input| {
        b.iter(|| part2::solve_puzzle_with_vec(input))
    });
